...
namespace CommonLibrary.Services.Sitemap
{
    public class SitemapBuilder
    {
        ...
        public void BuildSiteMap(TextWriter output)
        {
            ...
            List<SitemapUrl> allUrls = new List<SitemapUrl>();
        ...
        {
            if (System.IO.File.Exists(PortalSettings.HomeDirectoryMapPath + "Sitemap\\" + file))
            {
                // write the cached file to output
                StreamReader reader = new StreamReader(PortalSettings.HomeDirectoryMapPath + "/Sitemap/" + file, Encoding.UTF8);
        ...
        private bool IsChildPortal(PortalSettings ps, HttpContext context)
        {
            ...
            string serverPath = CommonLibrary.Common.Globals.GetAbsoluteServerPath(context.Request);

            if (arr.Count > 0)
            {
                PortalAliasInfo portalAlias = (PortalAliasInfo)arr[0];
                portalName = CommonLibrary.Common.Globals.GetPortalDomainName(ps.PortalAlias.HTTPAlias, null, true);
        ...
        private static List<SitemapProvider> _providers = null;
        ...
        private static void LoadProviders()
        {
            ...
            if (_providers == null)
            {
                ...
                lock (_lock)
                {
                    ...
                    foreach (KeyValuePair<string, SitemapProvider> comp in ComponentModel.ComponentFactory.GetComponents<SitemapProvider>())
                    {

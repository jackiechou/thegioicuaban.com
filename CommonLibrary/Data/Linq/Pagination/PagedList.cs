...
namespace CommonLibrary.Data.Linq.Pagination
{
    ...
    public class PagedList<T> : List<T>, IPageList<T>
    {
        ...
        //public PagedList(IEnumerable<T> source, int pageIndex, int pageSize, string identityColumnName, string sortExpression)
        //{
        //    Initialize(source.AsQueryable(), pageIndex, pageSize, identityColumnName, sortExpression);
        //}

        //public PagedList(IEnumerable<T> source, int pageIndex, int pageSize, string identityColumnName)
        //{
        //    Initialize(source.AsQueryable(), pageIndex, pageSize, identityColumnName);
        //}
        ...
        {            
            Initialize(source.AsQueryable(), pageIndex, pageSize, source.Count());
        }
        ...
        protected void Initialize(IQueryable<T> source, int pageIndex, int pageSize, int? totalCount)
        {
            ...
            var q2 = source.Skip(skip).Take(pageSize).Cast<T>().Future();
        ...
        protected void Initialize(IQueryable<T> source, int pageIndex, int pageSize, string identityColumnName)
        {
            if (pageIndex < 0)
                throw new ArgumentOutOfRangeException("pageIndex", "PageIndex cannot be below 0.");
            if (pageSize < 1)
        ...
        protected void Initialize(IQueryable<T> source, int pageIndex, int pageSize, string identityColumnName, string sortExpression)
        {
            if (pageIndex < 0)
                throw new ArgumentOutOfRangeException("pageIndex", "PageIndex cannot be below 0.");
            if (pageSize < 1)
        ...
        private void AddPage(IQueryable<T> source, int skip, int pageSize)
        {
            ...
            IQueryable<T> page = source.Skip(skip).Take(pageSize).Cast<T>();

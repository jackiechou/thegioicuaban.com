...
namespace CommonLibrary.Resources
{
    ...
    public static class ScriptLoader
    {
        ...
        /// </summary>
        public static string jQueryCdnUrl = "//ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js";

        /// <summary>
        /// Fallback Url if CDN can't be reached.
        /// </summary>
        public static string jQueryCdnFallbackUrl = "~/scripts/jquery.min.js";
        ...
        public static string jQueryLink(jQueryLoadModes jQueryLoadMode = jQueryLoadModes.Default, string url = null)
        {
            ...
            if (!string.IsNullOrEmpty(url))
                jQueryUrl = UrlUtils.ResolveUrl(url);
            else if (jQueryLoadMode == jQueryLoadModes.WebResource)
                jQueryUrl = script.GetClientScriptResourceUrl(typeof(ControlResources),
                                                              ControlResources.JQUERY_SCRIPT_RESOURCE);
            else if (jQueryLoadMode == jQueryLoadModes.ContentDeliveryNetwork)
            {
...
@"<script type=""text/javascript"">if (typeof(jQuery) == 'undefined')
        document.write(unescape(""%3Cscript src='{0}' type='text/javascript'%3E%3C/script%3E""));
                    ...
                    fallbackScript = string.Format(fallbackScript, UrlUtils.ResolveUrl(jQueryCdnFallbackUrl));
                }
            }
        ...
        public static string jQueryUiLink(jQueryLoadModes jQueryLoadMode = jQueryLoadModes.Default, string url = null)
        {
            ...
            if (!string.IsNullOrEmpty(url))
                jQueryUiUrl = UrlUtils.ResolveUrl(url);
            else if (jQueryLoadMode == jQueryLoadModes.WebResource)
...
@"<script type=""text/javascript"">if (typeof(jQuery) == 'undefined')
        document.write(unescape(""%3Cscript src='{0}' type='text/javascript'%3E%3C/script%3E""));
                    ...
                    fallbackScript = string.Format(fallbackScript, UrlUtils.ResolveUrl(jQueryUiLocalFallbackUrl));
                }
            }
        ...
        public static string jQueryUiCssLink(string theme = null)
        {
            ...
            var url = UrlUtils.ResolveUrl(jQueryUiCssBaseUrl);
        ...
        public static string wwJqueryLink(jQueryLoadModes jQueryLoadMode = jQueryLoadModes.Default, string url = null)
        {
            ...
            if (!string.IsNullOrEmpty(url))
                wwJQueryUrl = UrlUtils.ResolveUrl(url);
            else if (jQueryLoadMode == jQueryLoadModes.WebResource ||
                     jQueryLoadMode == jQueryLoadModes.ContentDeliveryNetwork)

                wwJQueryUrl = script.GetClientScriptResourceUrl(typeof(ControlResources),
                                                          ControlResources.WWJQUERY_SCRIPT_RESOURCE);

            if (string.IsNullOrEmpty(wwJQueryUrl))
                wwJQueryUrl = UrlUtils.ResolveUrl("~/scripts/ww.jquery.min.js");

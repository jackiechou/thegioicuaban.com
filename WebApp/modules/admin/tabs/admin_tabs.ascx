<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="admin_tabs.ascx.cs" Inherits="WebApp.modules.admin.tabs.admin_tabs" %>
...
<script language="javascript" type="text/javascript">
    var add_tab_path = "/modules/admin/tabs/admin_tabs_add.aspx?mode=add";
    var edit_tab_path = "/modules/admin/tabs/admin_tabs_edit.aspx?mode=edit&idx=";
    ...
    function AddModuleCancelScript() {
        var frame = $get('IframeModule');
        frame.src = loading_path;
        $find("AddModuleModalPopup").hide();
    }
    ...
    function ShowModuleModal() {
        ...
        if (SelectedValues != '') {
            if (SelectedItems.length == 1) {
                var path = "/modules/admin/tabs/admin_tab_module.aspx?id=";
                var frame = $get('IframeModule');
                frame.src = path + SelectedValues;
                $find('ModuleModalPopup').show();
            } else {
    ...
    function ShowAddModuleModal() {
        ...
        if (SelectedItems.length == 1) {
            var frame = $get('IframeAddModule');
            frame.src = "/modules/admin/tabs/admin_tab_module_add.aspx?idx=" + SelectedItems[0];
            $find('AddModuleModalPopup').show();

        } else {
            alert('Vui lòng chỉ check chọn 1 item để thêm thông tin');
            //document.getElementById('DivAddModuleWindow').style.display == 'none';     
            $find("AddModuleModalPopup").hide();
            return false;
        }
    }
...
<div class="wrap_gp">  
        <div class="commontool">
            ...
            <div class="toolbar">
                ...
                <asp:LinkButton ID="btnAddModule" runat="server" CssClass="btn" OnClientClick="ShowAddModuleModal();return false;"><span class="icon-32-new"></span>Thêm Module</asp:LinkButton>  
                <asp:LinkButton ID="btnShowModule" runat="server" CssClass="btn" OnClientClick="ShowModuleModal();return false;"><span class="icon-32-edit"></span>Show Module</asp:LinkButton>
        ...
        <div class="group_commands">              
            ...
            <div class="right">
                   ...
                   Chọn Portal :
                    ...
                    &nbsp;&nbsp;&nbsp;                 
                    Chọn Nhóm :
                     <asp:DropDownList ID="ddlIsSecure" CssClass="combobox" Width="147px" 
                       runat="server" onselectedindexchanged="ddlIsSecure_SelectedIndexChanged">                     
                    </asp:DropDownList>  
            </div>
        </div>
        ...
        <div class="group_panel">
         <asp:UpdatePanel ID="UpdatePanel1" EnableViewState="true" UpdateMode="Conditional" runat="server">
                 <Triggers>
                     <asp:PostBackTrigger ControlID="ddlIsSecure" />
                    ...
                    <asp:GridView ID="GridView1" runat="server" CssClass="table_list" DataKeyNames="TabID"
                     ...
                     <Columns>  
                         <asp:TemplateField HeaderText="x">
                            <EditItemTemplate>                                         
                                <asp:TextBox ID="txtIdx" runat="server" Text='<%# Bind("TabID") %>' Width="30px"></asp:TextBox>
                            </EditItemTemplate>
                            <ItemTemplate>
 ...
 <%----------------------------- ModalPopupExtender_Add -------------------------------------%>                                                                                                  
        ...
        <div id="DivAddWindow" style="display: none;" class="popupConfirmation">
            <iframe id="IframeAdd" frameborder="0" height="600" width="950" scrolling="no"></iframe>
        </div>

        <%----------------------------- ModalPopupExtender_Show Module -------------------------------------%>
	    <asp:Button ID="btnHiddenShowModule" runat="Server" style="display:none"  />
        <ajaxToolkit:ModalPopupExtender ID="btnShowModule_ModalPopupExtender" BackgroundCssClass="ModalPopupBG"
        runat="server" OkControlID="btOkay" CancelControlID="btCancel" TargetControlID="btnHiddenShowModule"
        PopupControlID="DivModuleWindow" Drag="true" PopupDragHandleControlID="PopupHeader" 
        OnOkScript="RefreshDataGrid();" OnCancelScript="ModuleCancelScript();" 
            BehaviorID="ModuleModalPopup">
        </ajaxToolkit:ModalPopupExtender>
        <div class="popup_Buttons" style="display: none">
            <input id="btOkay" value="Done" type="button" />
            <input id="btCancel" value="Cancel" type="button" />
        </div>
        <div id="DivModuleWindow" style="display: none;" class="popupConfirmation">
            <iframe id="IframeModule" frameborder="0" height="600" width="800" scrolling="no"></iframe>
        </div>  


        <%----------------------------- ModalPopupExtender_AddController -------------------------------------%>
        <ajaxToolkit:ModalPopupExtender ID="btnAddModule_ModalPopupExtender" BackgroundCssClass="ModalPopupBG"
        runat="server" OkControlID="btnOk" CancelControlID="btnClose" TargetControlID="btnAddModule"
        PopupControlID="DivAddModuleWindow" Drag="true" PopupDragHandleControlID="PopupHeader" 
        OnOkScript="RefreshDataGrid();" OnCancelScript="AddModuleCancelScript();" 
            BehaviorID="AddModuleModalPopup">
        </ajaxToolkit:ModalPopupExtender>
        <div class="popup_Buttons" style="display: none">
            <input id="btnOk" value="Done" type="button" />
            <input id="btnClose" value="Cancel" type="button" />
        </div>
        <div id="DivAddModuleWindow" style="display: none;" class="popupConfirmation">
            <iframe id="IframeAddModule" frameborder="0" height="740" width="450" scrolling="no"></iframe>
        </div>
        ...
        <div id="DivEditWindow" style="display: none;" class="popupConfirmation">
            <iframe id="IframeEdit" frameborder="0" height="600" width="950" scrolling="no">
            </iframe>
        </div> 
        <%----------------------------- Delete -------------------------------------------------------%>   
